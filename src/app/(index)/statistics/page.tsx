...
async function getMuscleGroupAnalysis() {
  ...
  const totalVolume = Object.values(groupedData).reduce(
...
async function getRPETrends() {
  ...
  return Object.entries(exerciseRPE)
    .map(([exercise, rpeEntries]) => {
      const avgRPE =
        rpeEntries.reduce((sum, entry) => sum + entry.rpe, 0) /
        rpeEntries.length
      ...
      const firstHalfAvg =
        firstHalf.length > 0
          ? firstHalf.reduce((sum, entry) => sum + entry.rpe, 0) /
            firstHalf.length
          : avgRPE
      const secondHalfAvg =
        secondHalf.length > 0
          ? secondHalf.reduce((sum, entry) => sum + entry.rpe, 0) /
            secondHalf.length
          : avgRPE
...
export default async function StatisticsPage() {
  ...
  const exerciseProgress = await getExerciseProgress(
    exercises[0]?.slug || 'press-de-banca'
  )
      ...
      <div className='flex items-center justify-between'>
        <div>
          ...
          <p className='text-muted-foreground'>
            Visualiza tu evolución y rendimiento en el gimnasio
          </p>
        </div>
      ...
      <div className='grid gap-4 md:grid-cols-2 lg:grid-cols-4'>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className='text-muted-foreground text-xs'>Por sesión</p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className='grid gap-6 lg:grid-cols-2'>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Volumen total de entrenamiento por semana
            </CardDescription>
          </CardHeader>
          <CardContent>
      ...
      <div className='grid gap-6 lg:grid-cols-2'>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Esfuerzo percibido promedio por ejercicio
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className='space-y-4'>
              {rpeTrends.length > 0 ? (
                ...
                <p className='text-muted-foreground text-sm'>
                  No hay datos de RPE disponibles
                </p>
              )}
            </div>
          </CardContent>
