...
export default function RoutineListFeature({
  initialRoutines,
  initialExercises,
}: RoutineListFeatureProps) {
  ...
  const handleDelete = async (routineId: number) => {
    ...
    try {
      ...
      if (result.deleted) {
        ...
        toast.info(
          'Rutina archivada. Tenía sesiones de entrenamiento asociadas.'
        )
      }
        ...
        <Card className='col-span-full'>
          <CardContent className='flex flex-col items-center justify-center py-12 text-center'>
            ...
            <p className='text-muted-foreground mb-4 max-w-sm'>
              Creá tu primera rutina para empezar a organizar tus
              entrenamientos.
            </p>
            <Button onClick={() => router.push('/routines/new')}>
              <Plus className='mr-2 h-4 w-4' />
