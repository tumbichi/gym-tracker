...
import { DraftSession, DRAFT_SESSION_VERSION } from '../../types/draft-session'
...
function WorkoutSessionProvider({
  ...
  const [draftStatus, setDraftStatus] = useState<
    'idle' | 'recovering' | 'active' | 'committing'
  >('idle')
  ...
  useEffect(() => {
    if (draftStatus !== 'active' || exercises.length === 0 || !loadedDraft)
      return
  ...
  // History management
  const setExercisesWithHistory = useCallback(
    (
      newExercises:
        | WorkoutExercise[]
      ...
      setExercises((prev) => {
        const newValue =
          typeof newExercises === 'function' ? newExercises(prev) : newExercises
  ...
  const finishWorkout = async (notes: string): Promise<CommitSessionResult> => {
    if (!timer.startDate) {
    ...
    const payload: CommitSessionPayload = {
      ...
      durationSeconds:
        (finishedAt.getTime() - timer.startDate.getTime()) / 1000,
      routineId: routineDay?.routineId,
      notes: notes,
      exercises: exercises
        .filter((ex: WorkoutExercise) =>
          ex.sets.some((set: SetEntry) => set.completed)
        )
        .map(
          (
            ex: WorkoutExercise,
