...
export const database = {
  ...
  workoutSession: {
    findMany: async ({ include, where, select }: { include?: any, where?: Prisma.WorkoutSessionWhereInput, select?: Prisma.WorkoutSessionSelect} = {}) => {
      try {
        if (prismaClient) {
  ...
  setEntry: {
    findMany: async ({ where, include }: { where?: any; include?: any } = {}) => {
      ...
      return mockDb.setEntry.findMany({ where })
    },
    ...
    groupBy: async ({ by, where }: { by: string[]; where?: any } = {}) => {
      try {
        if (prismaClient) {
      ...
      return mockDb.setEntry.groupBy({ by, where })
    },
  },
