...
async function getPersonalRecords() {
  ...
  for (const entry of setEntries) {
    ...
    if (entry.weightKg > currentRecord.weightKg) {
      ...
      entry.weightKg === 0 &&
      entry.repsDone > currentRecord.repsDone
    ) {
      recordsByExercise[entry.exerciseId] = entry as SetEntryWithRelations
    }
  }

  const records = Object.values(recordsByExercise).map(
    (record: SetEntryWithRelations) => {
      const improvement = '+5kg'
      const isRecent = record.workoutExercise?.session?.date
        ? new Date(record.workoutExercise.session.date).getTime() >
      ...
      return {
        ...
        date:
          record.workoutExercise?.session?.date?.toISOString() ??
