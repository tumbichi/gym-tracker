...
mockSessions[0].workoutExercises = [
  mockWorkoutExercises[0],
  mockWorkoutExercises[1],
]
...
export const db = {
  exercise: {
    ...
    findUnique: async ({ where }: { where: { id: number } }) =>
      mockExercises.find((e) => e.id === where.id) || null,
  ...
  setEntry: {
    ...
    aggregate: async ({ where, _sum }: { where?: any; _sum?: any } = {}) => {
      const totalWeight = mockSetEntries.reduce(
    ...
    groupBy: async () => {
      const uniqueExercises = [
        ...new Set(mockSetEntries.map((entry) => entry.exerciseId)),
      ]
  ...
  routineDay: {
    findMany: async (
      { where }: { where: { routineId: number } } = { where: { routineId: 1 } }
    ) => {
