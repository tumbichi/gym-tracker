...
export class WorkoutSessionPage {
  ...
  constructor(page: Page) {
    ...
    this.emptySessionMessage = page.getByText('Agrega tu primer ejercicio para comenzar');
    this.loadingScreen = page.locator('div.flex.flex-col.items-center.justify-center.h-screen.bg-background'); // Selector for the entire LoadingScreen div
  }
  ...
  async startWorkout(expectModal: boolean = false) {
    await this.page.getByRole('button', { name: 'Comenzar Sesión Libre' }).click();
    ...
    // Removed waitForMainContent here, will handle in tests
    await this.page.getByText('Entrenamiento Libre').waitFor({ state: 'visible', timeout: 10000 }); // Wait for the main title to be visible
    if (!expectModal) {
  ...
  async setDraftInLocalStorageBeforeNavigation(data: any) {
    ...
    await this.context.addInitScript((draftData) => {
      localStorage.setItem('gym-tracker:draft-session', JSON.stringify(draftData));
    }, data);
  }
  ...
  async expectModalVisible() {
    ...
    await expect(this.page.getByText('Borrador de sesión encontrado')).toBeVisible({ timeout: 10000 });
  }

  async expectModalNotVisible() {
    await expect(this.draftModal).not.toBeVisible({ timeout: 10000 });
    await expect(this.page.getByText('Borrador de sesión encontrado')).not.toBeVisible({ timeout: 10000 });
  }
  ...
  async discardDraft() {
    await this.page.getByRole('button', { name: 'Descartar e iniciar nueva' }).click();
    await this.draftModal.waitFor({ state: 'hidden', timeout: 10000 }); // Wait for the modal to disappear
    await this.expectModalNotVisible();
  }
  ...
  async expectDraftToBeCleared() {
    const draft = await this.page.evaluate(() => localStorage.getItem('gym-tracker:draft-session'));
  ...
  async completeSet(exerciseName: string, setIndex: number, reps: string, weight: string) {
    const normalizedName = this.normalizeExerciseNameForTestId(exerciseName);
    const exerciseSection = this.page.locator(`[data-test-id="exercise-section-${normalizedName}"]`);
    const setRow = exerciseSection.locator(`[data-test-id="set-row-${setIndex}"]`);
    ...
    await expect(setRow.locator('[data-test-id="completed-set-indicator"]')).toBeVisible();
    await this.page.waitForLoadState('networkidle'); // Wait for set to be completed and rendered
  }

  async editSet(exerciseName: string, setIndex: number, newReps: string, newWeight: string) {
    const normalizedName = this.normalizeExerciseNameForTestId(exerciseName);
    const exerciseSection = this.page.locator(`[data-test-id="exercise-section-${normalizedName}"]`);
    const setRow = exerciseSection.locator(`[data-test-id="set-row-${setIndex}"]`);
    ...
    await expect(setRow.locator('[data-test-id="reps-display"]')).toHaveText(newReps);
    await expect(setRow.locator('[data-test-id="weight-display"]')).toHaveText(newWeight);
  ...
  async expectExerciseAndSetData(exerciseName: string, setIndex: number, reps: string, weight: string) {
    const normalizedName = this.normalizeExerciseNameForTestId(exerciseName);
    const exerciseSection = this.page.locator(`[data-test-id="exercise-section-${normalizedName}"]`);
    ...
    await expect(setRow.locator('input[type="number"]').nth(0)).toHaveValue(weight, { timeout: 10000 });
    await expect(setRow.locator('input[type="number"]').nth(1)).toHaveValue(reps, { timeout: 10000 });
    await expect(setRow.locator('[data-test-id="completed-set-indicator"]')).toBeVisible({ timeout: 10000 });
  }
}
