...
export class LogWorkoutPage {
  ...
  async goto() {
    ...
    await expect(this.page.getByRole('heading', { name: 'Registrar Entrenamiento' })).toBeVisible();
  }

  // Elements for starting a new workout
  get startTodayWorkoutButton() {
    return this.page.getByRole('button', { name: 'Comenzar Entrenamiento de Hoy' });
  }
  ...
  get draftRecoveryModalTitle() {
    return this.page.getByRole('heading', { name: 'Borrador de sesión encontrado' });
  }
