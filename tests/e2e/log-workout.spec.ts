...
test.describe('Log Workout Session E2E Tests', () => {
  ...
  test('should initiate free workout session successfully (no draft, no modal)', async ({ page }) => {
  ...
  test('should resume session from draft without modal when using recover=true', async ({ page }) => {
    await workoutSessionPage.setDraftInLocalStorageBeforeNavigation(dummyDraftData);
  ...
  test('should show modal, then continue draft when starting new session with existing draft', async ({ page }) => {
    await workoutSessionPage.setDraftInLocalStorageBeforeNavigation(dummyDraftData);
  ...
  test('should show modal, then discard draft and start new session', async ({ page }) => {
    await workoutSessionPage.setDraftInLocalStorageBeforeNavigation(dummyDraftData);
    await page.goto('/log-workout/session');
    // Wait for the page to be fully loaded and for draft detection
    await page.waitForLoadState('networkidle');
  ...
  test('should persist draft after reload', async ({ page }) => {
    ...
    const normalizedExerciseName = exerciseName
      ...
      .replace(/[\u0300-\u036f]/g, '')
      .replace(/\s+/g, '-');
    await workoutSessionPage.expectExerciseAndSetData(normalizedExerciseName, 1, '10', '50');
  });
  ...
  test('should complete a set for an exercise', async () => {
    ...
    const normalizedName = exerciseName
      ...
      .replace(/[\u0300-\u036f]/g, '')
      .replace(/\s+/g, '-');
    const exerciseSection = workoutSessionPage.page.locator(`[data-test-id="exercise-section-${normalizedName}"]`);
